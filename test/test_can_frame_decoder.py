...
TEST_VALID_FRAME_1 = bytearray(
    b'\xFF\x00\x53\x00\x02\x80\x41\x80\x6D'
    b'\x80\x6C\x00\x45\x00\x04\x80\x6F\x80\x23\x80\x47\xFE')

TEST_VALID_FRAME_2 = bytearray(
    b'\xFF\x00\x53\x00\x06\x80\x2B\x80\x72'
    b'\x80\x6B\x00\x45\x00\x04\x80\x6F\x80\x23\x80\x47\xFE')

TEST_MISSING_START_FRAME = bytearray(
    b'\x00\x53\x00\x02\x80\x38\x80\x6F\x80'
    b'\x7B\x00\x45\x00\x04\x80\x6F\x80\x23\x80\x47\xFE')

TEST_MISSING_END_FRAME = bytearray(
    b'\xFF\x00\x53\x00\x02\x80\x38\x80\x6F'
    b'\x80\x7B\x00\x45\x00\x04\x80\x6F\x80\x23\x80\x47')

TEST_EMBEDDED_FRAME_IN_MESSAGE = bytearray(
    b'\x6F\x80\x23\x80\x47\xFE\xFF\x00\x53\x00\x02\x80'
    b'\x22\x80\x6F\x80\x5F\x00\x45\x00\x04\x80\x6F\x80'
    b'\x23\x80\x47\xFE\xFF\x00\x53\x00\x03\x80\x26')
...
# pylint: disable=redefined-outer-name
    ...
    def test_get_frame_id(self):
        ...
        for index in range(PACKAGE_SIZE):
            assert CanFrameDecoder.get_frame_id(
                TEST_VALID_PACKAGE[index]) == \
                (DEVICE_ID + index), 'frame ' + str(index) + ' failed'
