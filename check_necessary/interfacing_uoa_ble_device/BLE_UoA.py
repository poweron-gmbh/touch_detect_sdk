# This Python program creates a GUI which interfaces with the UoA BLE device.
...
# NOTIFY_UUID = "0000fe42-8e22-4541-9d4c-21edae82ed19"  # For STM32 BLE
NOTIFY_UUID = "fb33608b-a9b9-4aa2-8421-534c8dff02d0"    # For UoA BLE
...
BLE_device_name = "PowerOn "
BLE_device_found = False
sensor_array_size = 32
data2 = np.zeros([8, 4])
data_ready = np.zeros([8, 4])
...
def plot_graph(data_plot):
    sensor_row = ["Row1", "Row2", "Row3", "Row4", "Row5", "Row6", "Row7", "Row8"]
    sensor_col = ["Col1", "Col2", "Col3", "Col4"]

    ax.clear()
    ax.imshow(data_plot, cmap='Purples', vmin=0, vmax=255)

    voltage = np.zeros([8, 4])
    resistance = np.zeros([8, 4])

    for i in range(len(sensor_row)):
        for j in range(len(sensor_col)):
            voltage[i, j] = data_plot[i, j] / 255 * 3.3
            resistance[i, j] = round((511 * voltage[i, j] / (3.3 - voltage[i, j])))
...
BLE_Connect_btn = tk.Button(BLE_Header, textvariable=BLE_Connect_text, command=ble_connect_cmd)
...
def notification_handler(sender, data):
    ...
    global data_ready
    data_ready = np.array(data2).reshape(-1, 4)
    data_total = data2
...
BLE_Receive_Data_btn = tk.Button(BLE_Header, textvariable=BLE_Receive_Data_text, state=tk.DISABLED,
...
BLE_Stop_Data_btn = tk.Button(BLE_Header, textvariable=BLE_Stop_Data_text, command=stop_ble_cmd, state=tk.DISABLED)
...
BLE_Disconnect_btn = tk.Button(BLE_Header, textvariable=BLE_Disconnect_text, command=ble_disconnect_cmd,
                               state=tk.DISABLED)
BLE_Disconnect_text.set("Disconnect")
BLE_Disconnect_btn.grid(column=0, row=2, padx=10, pady=10)

# Header - Data logging [frame]
Data_Log_Header = tk.LabelFrame(root, text="Data Logging", width=250, height=200, padx=10, pady=10)
...
Log_data_var.set(0)
Data_Log_Yes_btn = tk.Radiobutton(Data_Log_Header, text="Yes", variable=Log_data_var, value=1, state=tk.DISABLED,
                                  command=data_logger_cmd)
Data_Log_Yes_btn.grid(column=0, row=1)
Data_Log_No_btn = tk.Radiobutton(Data_Log_Header, text="No", variable=Log_data_var, value=0, state=tk.DISABLED,
                                 command=data_logger_cmd)
Data_Log_No_btn.grid(column=0, row=2)
...
Exit_btn = tk.Button(root, textvariable=Exit_text, command=exit_btn_command)
Exit_text.set("Exit")
...
def plot_initial_graph():
    plot_graph(np.array(data2).reshape(-1, 4))
    fig.tight_layout()
